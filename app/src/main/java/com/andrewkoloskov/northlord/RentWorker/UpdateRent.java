...
public class UpdateRent extends DialogFragment {
    EditText start;
    EditText EditEnd;
    EditText Cost;
    EditText Name;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rent = new Rent();
        ((TextView)view.findViewById(R.id.updaterenttitle)).setText("Изменить условия");
        Name=view.findViewById(R.id.AddRentName);
        Cost = view.findViewById(R.id.AddRentCost);
        Cost.setEnabled(false);
        rent.id=getArguments().getInt("id");rentcost=getArguments().getInt("rent");
...
//                        start.setText(dateAndTime.get(Calendar.YEAR) + " " + dateAndTime.get(Calendar.MONTH) + " " + dateAndTime.get(Calendar.DAY_OF_MONTH) + " " + dateAndTime.get(Calendar.HOUR_OF_DAY) + " " + dateAndTime.get(Calendar.MINUTE) + " ");
    ...
    public class AddRentTask extends AsyncTask<Void, Void, Boolean> {
        ...
        AddRentTask() {
            try {
                CarInfoActivity c=(CarInfoActivity) getActivity();
                log=c.login;
                pas=c.password;
            ...
            try {
                if(rent.cost>0&!Name.getText().toString().equals("")){
                    ...
                    RequestBody formBody = new FormBody.Builder()
                            .add("login", URLEncoder.encode(log,"UTF-8"))
                            .add("password", URLEncoder.encode(pas,"UTF-8"))
                            .add("id", rent.id+"")
...
//            ((MainActivity)getActivity()).showProgress(false);
            ...
            if (success) {
                try {
                    ...
                    try {
                        MainActivity m=(MainActivity)getActivity();
                        ((LastRentsFragment)m.last).update();
                    }catch (Exception e1){e1.printStackTrace();}
