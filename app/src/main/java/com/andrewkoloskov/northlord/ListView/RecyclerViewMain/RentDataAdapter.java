...
public class RentDataAdapter extends Adapter {
    private final LayoutInflater inflater;
    private final List<Card> values;
    private Context context;
    FragmentManager fragmentManager;

    public RentDataAdapter(Context context, List<Card> rents, FragmentManager fragmentManager) {
        this.values = rents;
    ...
    public ViewH onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view = inflater.inflate(R.layout.rent_item, viewGroup, false);
        return new RentDataAdapter.ViewHolder(view);
    }
        ...
        for (int i = 0; i < values.size(); i++) {
            if (values.get(i).checked) {
                ...
                try {
                    ...
                    object.put("cost", ((Rent) values.get(i)).cost);
                    selected.add(object);
                } catch (Exception e) {
                    e.printStackTrace();
                }
